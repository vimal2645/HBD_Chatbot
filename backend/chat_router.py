...
async def handle_chat_query(req, session_phone: Optional[str], session_email: Optional[str], lang: str):
    ...
    if flow_state == "START" or q_lower == "start new search":
        ...
        if any(x in q_lower for x in ["business listing", "listings", "explore business", "business"]):
        ...
        # Products branch
        elif any(x in q_lower for x in ["product", "products", "shop", "buy", "item"]):
            with mysql_ctx() as conn:
                cur = conn.cursor(dictionary=True)
    ...
    if is_my_biz_query:
        ...
        try:
            ...
            if "manage product" in q_lower or "show product" in q_lower:
                ...
                suggs = [{"title": "Update Profile", "action": "query_rewrite", "query": "update business"}]
                resp = {
                    "type": "database",
                    "data": [mapped],
